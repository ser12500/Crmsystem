...
interface ICustomerFormState extends Pick<
  ICustomer,
...
<template>
  <div class="p-10">
    ...
    <form class="form" @submit="onSubmit">
      ...
      <UiInput v-model="email" placeholder="Email" v-bind="emailAttrs" type="text" class="input" />
      <UiInput
        v-model="fromSource"
        placeholder=""
      ...
      <div class="grid w-full max-w-sm items-center gap-1.5 input">
        <label>
          ...
          <UiInput
            type="file"
            :onchange="
              (e: InputFileEvent) => e?.target?.files?.length && uploadImage(e.target.files[0])
            "
